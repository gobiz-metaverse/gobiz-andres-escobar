...
export default class Match extends React.Component {
    ...
    loadBets() {
        ...
        MatchService.getBetsByMatchAndUser(this.state.match.code, user.preferred_username).then((response) => {
            this.setState({
    ...
    render() {
        ...
        const dataSource = [
            ...
            {
                title: "Đã đặt",
                home: this.state.bets.filter((b) => b.code === '1').reduce((prev, curr) => {
                    return prev + curr.money
                }, 0),
                fair: this.state.bets.filter((b) => b.code === 'x').reduce((prev, curr) => {
                    return prev + curr.money
                }, 0),
                away: this.state.bets.filter((b) => b.code === '2').reduce((prev, curr) => {
                    return prev + curr.money
                }, 0),
                type: "text",
            },
        ...
        return (
            <StandardLayout>
                <PageHeader
                    ...
                    tags={<Tag
                        color={moment(this.state.match.startTime).isBefore(moment()) ? 'gray' : 'blue'}>{moment(this.state.match.startTime).isBefore(moment()) ? 'Đã diễn ra' : 'Sắp diễn ra'}</Tag>}
