...
import { Input, Button, message, notification, Modal } from 'antd'
...
export default class Match extends React.Component {
	...
	confirmBet = () => {
		...
		let handleResponse = (response) => {
			...
			else message.error('Không bet được, vui lòng kiểm tra lại thông tin bạn nhập')
		}
	...
	render() {
			...
			return (
				<StandardLayout>
					<Modal
						...
						footer={null}
					>
						<p>Bạn chưa đăng nhập hoặc đã hết phiên.</p>
						<a className={classes.loginLink} onClick={() => (window.location = LOGIN_URL)}>
							Đăng nhập ngay thôi!
						</a>
					</Modal>
				</StandardLayout>
		...
		return (
			<StandardLayout>
				...
				<h1>
					{this.state.match.homeTeam.name} vs {this.state.match.awayTeam.name}
				</h1>
				Hướng dẫn chơi: bạn hãy điền số xu bạn muốn đặt (đơn vị tính nghìn, tức là nếu bạn nhập 100 tương đương
				bạn muốn đặt 100k). Sau đó click vào nút "Xác nhận kèo" để xác nhận tham gia.
				<br />
				...
				<br />
				Việc bạn tham gia chơi trò chơi này mục đích chính là để đóng góp cho quỹ liên hoan xem World Cup, vui
				lòng giữ tinh thần vui vẻ là chính.
				<table className={'table'} style={{ maxWidth: '800px' }}>
					...
					<tr>
						...
						<td className={'text-right'}>
							{homeOdds ? strip(homeOdds.ratio * this.state.homeBet * 1000) : null}
						</td>
						<td className={'text-right'}>
							{drawOdds ? strip(drawOdds.ratio * this.state.drawBet * 1000) : null}
						</td>
						<td className={'text-right'}>
							{awayOdds ? strip(awayOdds.ratio * this.state.awayBet * 1000) : null}
						</td>
					</tr>
					<tr>
						<th>Số xu lãi khi thắng</th>
						<td className={'text-right'}>
							{homeOdds ? strip((homeOdds.ratio - 1) * this.state.homeBet * 1000) : null}
						</td>
						<td className={'text-right'}>
							{drawOdds ? strip((drawOdds.ratio - 1) * this.state.drawBet * 1000) : null}
						</td>
						<td className={'text-right'}>
							{awayOdds ? strip((awayOdds.ratio - 1) * this.state.awayBet * 1000) : null}
						</td>
					</tr>
					<tr>
						<td colSpan={4} className={'text-right'}>
							<Button type={'primary'} onClick={this.confirmBet}>
								Xác nhận kèo
							</Button>
						</td>
					</tr>
