...
export default class M6Client extends httpClient {
    ...
    static async requestPromise(requestConfig, token, handleResponse = null, cache_ttl = 0) {
        return new Promise(function(resolve, reject) {
            if (cache_ttl) {
                let data = M6Client.cache.get(requestConfig.endpoint, null);
                if (data) {
                    return resolve({
                        status: 200,
                        body: data
            ...
            M6Client.request(requestConfig, token, (error, response)=> {
                if (error) {
                    reject(error)
                }
                else {
                    ...
                    if (cache_ttl) {
                        M6Client.cache.set(requestConfig.endpoint, response.body, cache_ttl)
                    }
